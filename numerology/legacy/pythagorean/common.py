...
class Functions:
    ...
    def keep_valid_letters(cls, string: str, alphabet: Dict) -> str:
        ...
        if len(valid_letters) == 0:
            print(
                f"{Color.WARNING}The string supplied contains no valid character.{Color.ENDC}"
            )
            return None
        else:
    ...
    def is_valid_date(cls, date_supplied: str) -> bool:
        ...
        if not re.match(r"(\d){4}-(\d){2}-(\d){2}", date_supplied):
            print(
                f"{Color.WARNING}Invalid date format. Expected: yyyy-mm-dd.{Color.ENDC}"
            )
            return False
        else:
            try:
                ...
                print(
                    f"{Color.WARNING}Invalid date: {str(err).capitalize()} (Example of valid date: '2020-11-25').{Color.ENDC}"
                )
                return False
            return True
    ...
    def print_beautiful_dict(cls, dictionary: Dict, json_format: bool = True):
        ...
        if json_format:
            print(
                f"{Color.OKGREEN}{json.dumps(dictionary, indent=4, sort_keys=False, ensure_ascii=False)}{Color.ENDC}"
            )
        else:
            print(f"{Color.OKGREEN}{dictionary}{Color.ENDC}")
