...
def pythagorean_sum(
    iterable: Iterable[int], master_number: bool = True, upper_bound: int = 9
) -> int | None:
    """Make the pythagorean numerology sum of an itarable of integers.
    ...
    Args:
        iterable (Iterable[int]): Tuple to sum up
    ...
    Returns:
        int: Calculated numerology value.
    """
    if not iterable:
        logger.warning("Empty iterable.")
        return None

    sum: float = math.fsum(iterable)
